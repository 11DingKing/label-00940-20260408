...
class GrpcClientFactory
{
     ...
     * 获取 OrderService 客户端
     */
    public function getOrderServiceClient(): OrderServiceClient
    {
        return $this->getClient(
            OrderServiceClient::class,
            'order.OrderService',
            env('ORDER_SERVICE_HOST', 'order-service'),
            (int) env('ORDER_SERVICE_PORT', 9504)
        );
    }
    ...
    protected function getClient(string $clientClass, string $serviceName, string $defaultHost, int $defaultPort): mixed
    {
        ...
        $this->logger->debug('gRPC client created', [
            'client' => $clientClass,
            'address' => $address,
        ]);
